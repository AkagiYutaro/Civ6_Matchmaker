...
class BanPickPhaseManager:
    ...
    async def announce_results(self):
        ...
        # ピック可能(生き残った)文明
        survivors = [L for L in self.all_leaders if L["指導者名"] not in final_banned]
        
        embed = discord.Embed(title="🎉 CIV6 BAN/PICK 最終結果", color=discord.Color.gold())
        
        # BANリスト表示
        ban_text = f"**【🌐 グローバルBAN】**\n" + ("、".join(self.global_banned) or "なし") + "\n\n"
        ban_text += f"**【🔵 チームAのBAN】**\n" + ("、".join(self.banned_a) or "なし") + "\n\n"
        ban_text += f"**【🔴 チームBのBAN】**\n" + ("、".join(self.banned_b) or "なし")
...
class TargetBanView(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user.id != self.rep_id:
...
class GlobalBanView(discord.ui.View):
    ...
    async def callback_a(self, interaction: discord.Interaction):
        if interaction.user.id != self.rep_a and interaction.user.id != self.host.id:
    ...
    async def callback_b(self, interaction: discord.Interaction):
        if interaction.user.id != self.rep_b and interaction.user.id != self.host.id:
    ...
    async def check_ready(self, interaction: discord.Interaction):
        if self.select_a.disabled and self.select_b.disabled:
            ...
            available_leaders = [L for L in self.all_leaders if L["指導者名"] not in banned_global]
            # 名前でソート
            ...
            await interaction.response.edit_message(content=f"🌐 **グローバルBANが確定しました: {', '.join(banned_global)}**\n続いて各チームのBANフェーズに移行します。", view=None)
            
            # A用とB用のパネルを2つのメッセージに分けて送信 (25個制限を回避するため)
...
class BanPickStartView(discord.ui.View):
    ...
    async def start_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != self.host.id:
            return await interaction.response.send_message("ホストのみが開始できます。", ephemeral=True)
        ...
        # グローバルBAN候補リストの抽出
        global_pool = [L for L in all_leaders if L.get("グローバルBAN候補") == 1]
        if not global_pool:
            # 設定がなければランダムで10個選出
            global_pool = random.sample(all_leaders, min(10, len(all_leaders)))
        ...
        embed = discord.Embed(title="【🌐 フェーズ1: グローバルBAN】", description=f"両チームの代表者(<@{rep_a_id}>, <@{rep_b_id}>)は、以下のメニューから1つずつ除外する文明を選択してください。", color=discord.Color.red())
